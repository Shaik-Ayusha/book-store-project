                ...
                orders.length === 0 ? (<div>No orders found!</div>) : (<div>
                    {
                        orders.map((order, index) => (
                            <div key={order._id} className="border-b mb-4 pb-4">
                                ...
                                <p className="text-gray-600">Total Price: {formatINR(order.totalPrice)}</p>
                               <h3 className="font-semibold mt-2">Products:</h3>

        <div className="space-y-2">
 {order.productIds?.map((book) => (
  <div key={book?._id || book}>
    <p>Title: {book?.title || "No title"}</p>
        <p>Price: {formatINR(book?.newPrice || "N/A")}</p>
  </div>
))}
</div>
